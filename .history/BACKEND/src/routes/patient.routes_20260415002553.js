  ...
  try {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
      return res.status(400).json({
        message: "Device location (latitude & longitude) is required",
      });
    }
...
validMachines.forEach((machine) => {
  const hospital = machine.hospitalId;
  if (!hospital) return;
  ...
  // 🔥 Filter only available slots for that date
  const availableSlots = machine.slots.filter(
    (slot) =>
      slot.date === cleanDate &&
      slot.availability_status === "available"
  );
